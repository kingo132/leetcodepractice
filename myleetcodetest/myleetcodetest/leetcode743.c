...
int minIdxFromSet(int* isIncluded, int* minWeightAry, int n) {
    ...
    int minIdx = -1;
    for (int i = 0; i < n; i++) {
        if (! isIncluded[i] && minWeightAry[i] < min) {
            min = minWeightAry[i];
            minIdx = i;
        }
    }
...
int getWeight(int u, int v, int** times, int timesSize) {
    for (int i = 0; i < timesSize; i++) {
        if (times[i][0] == u && times[i][1] == v) {
            return times[i][2];
        }
    }
...
int networkDelayTimeOld(int** times, int timesSize, int* timesColSize, int n, int k){
    int* parent = (int*)malloc(n * sizeof(int));
    int* costFromK = (int*)malloc(n * sizeof(int));
    int* isIncluded = (int*)malloc(n * sizeof(int));
    int* minWeightAry = (int*)malloc(n * sizeof(int));
    for (int i = 0; i < n; i++) {
        parent[i] = -1;
        costFromK[i] = -1;
        isIncluded[i] = false;
        minWeightAry[i] = INT_MAX;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (! isIncluded[j]) {
                int w = getWeight(idx + 1, j + 1, times, timesSize);
                if (w >= 0 && w < minWeightAry[j]) {
                    minWeightAry[j] = w;
                    parent[j] = idx;
...
void dumpQueue(QUENODE* que, int front, int tail, int queSize) {
    printf("Dump queue: ");
    for (int i = 0; i < queSize; i++) {
        int idx = (front + i) % MAX_QUE_SIZE;
        printf("(%d,%d) ", que[idx].v + 1, que[idx].cost);
    }
    printf("\n");
}

int networkDelayTimeWithQueue(int** times, int timesSize, int* timesColSize, int n, int k) {
    QUENODE* que = (QUENODE*)malloc(MAX_QUE_SIZE * sizeof(QUENODE));
    ...
    while (queSize > 0) {
        ...
        for (int j = 0; j < timesSize; j++) {
            if (times[j][0] == i + 1) {
                int child = times[j][1] - 1;
                
                int newCost = costFromK[i] + times[j][2];
                if (newCost < costFromK[child]) {
                    costFromK[child] = newCost;
                    parent[child] = i;
                    if (visited[child] == 0) {
                        que[tail].v = child;
                        ...
                        while (queSize > 1 && idxTmp > 0) {
                            ...
                            int prevII = (front + idxTmp - 1) % MAX_QUE_SIZE;
                            if (que[ii].cost < que[prevII].cost) {
                                int tmpV = que[prevII].v;
                                int tmpCost = que[prevII].cost;
                                que[prevII].v = que[ii].v;
                                que[prevII].cost = que[ii].cost;
                                que[ii].v = tmpV;
                                que[ii].cost = tmpCost;
                            } else {
                                break;
    ...
    for (int i = 0; i < n; i++) {
        printf("%d,%d,%d ", visited[i], costFromK[i], parent[i]);
    }
    printf("\n");
...
int networkDelayTimeChatGPT(int** times, int timesSize, int* timesColSize, int n, int k) {
    int* que = (int*)malloc(MAX_QUE_SIZE * sizeof(int));
    int* visited = (int*)malloc(n * sizeof(int));
    int* costFromK = (int*)malloc(n * sizeof(int));
    int front = 0, tail = 0;
    int queSize = 0;
    ...
    int maxCost = 0;
    for (int i = 0; i < n; i++) {
        if (costFromK[i] == INT_MAX) {
            maxCost = -1;
            break;
        }
        if (costFromK[i] > maxCost) {
            maxCost = costFromK[i];
        }
    }
...
int minCostFromK(int* costFromK, int* isVisited, int n) {
    int min = INT_MAX;
    int minIdx = -1;
    for (int i = 0; i < n; i++) {
        if (isVisited[i] == 0 && costFromK[i] < min) {
            min = costFromK[i];
            minIdx = i;
        }
    }
...
int networkDelayTimeWithoutQueue(int** times, int timesSize, int* timesColSize, int n, int k) {
    int* costFromK = (int*)malloc(n * sizeof(int));
    ...
    for (int i = 0; i < n; i++) {
        int idx = minCostFromK(costFromK, isVisited, n);
        if (idx < 0) {
        ...
        isVisited[idx] = 1;
        for (int j = 0; j < timesSize; j++) {
            int neighbor = times[j][1] - 1;
            if (times[j][0] == idx + 1 && isVisited[neighbor] == 0) {
                int newCost = costFromK[idx] + times[j][2];
                if (newCost < costFromK[neighbor]) {
                    costFromK[neighbor] = newCost;
                }
            }
        }
    }
...
int leetcode743Main(void) {
    ...
    int** timesPtr = (int**)malloc(timesSize * sizeof(int*));
    for (int i = 0; i < timesSize; i++) {
        timesPtr[i] = times[i];
    }

    int result = networkDelayTimeWithoutQueue(timesPtr, timesSize, timesColSize, n, k);
